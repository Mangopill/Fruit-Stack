...
public class TvfmpoitRecipeBuilder
{
	...
	private TvfmpoitRecipeBuilder(ItemLike resultIn, int count, int cookingTime, float experience, @Nullable ItemLike container) {
	...
	public static TvfmpoitRecipeBuilder tripodVesselForMakingPillsOfImmortalityRecipe(ItemLike mainResult, int count, int cookingTime, float experience) {
		return new TvfmpoitRecipeBuilder(mainResult, count, cookingTime, experience, null);
	}

	public static TvfmpoitRecipeBuilder tripodVesselForMakingPillsOfImmortalityRecipe(ItemLike mainResult, int count, int cookingTime, float experience, ItemLike container) {
		return new TvfmpoitRecipeBuilder(mainResult, count, cookingTime, experience, container);
	}

	public TvfmpoitRecipeBuilder addIngredient(TagKey<Item> tagIn) {
		return addIngredient(Ingredient.of(tagIn));
	}

	public TvfmpoitRecipeBuilder addIngredient(ItemLike itemIn) {
		return addIngredient(itemIn, 1);
	}

	public TvfmpoitRecipeBuilder addIngredient(ItemLike itemIn, int quantity) {
	...
	public TvfmpoitRecipeBuilder addIngredient(Ingredient ingredientIn) {
		return addIngredient(ingredientIn, 1);
	}

	public TvfmpoitRecipeBuilder addIngredient(Ingredient ingredientIn, int quantity) {
		for (int i = 0; i < quantity; ++i) {
	...
	public TvfmpoitRecipeBuilder unlockedBy(String criterionName, CriterionTriggerInstance criterionTrigger) {
		advancement.addCriterion(criterionName, criterionTrigger);
		return this;
	}

	public TvfmpoitRecipeBuilder unlockedByItems(String criterionName, ItemLike... items) {
		return unlockedBy(criterionName, InventoryChangeTrigger.TriggerInstance.hasItems(items));
	}

	public TvfmpoitRecipeBuilder unlockedByAnyIngredient(ItemLike... items) {
	...
	public void build(Consumer<FinishedRecipe> consumerIn) {
		...
		build(consumerIn, fruitstack.MODID + ":cooking/" + location.getPath());
	}
	...
	public void build(Consumer<FinishedRecipe> consumerIn, ResourceLocation id) {
		if (!advancement.getCriteria().isEmpty()) {
			...
			consumerIn.accept(new TvfmpoitRecipeBuilder.Result(id, result, count, ingredients, cookingTime, experience, container, advancement, advancementId));
		} else {
			consumerIn.accept(new TvfmpoitRecipeBuilder.Result(id, result, count, ingredients, cookingTime, experience, container));
		}
	}
